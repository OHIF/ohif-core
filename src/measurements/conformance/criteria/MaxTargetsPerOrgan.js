...
export class MaxTargetsPerOrganCriterion extends BaseCriterion {
    ...
    evaluate(data) {
        ...
        _.each(data.targets, target => {
            ...
            const { location, lesionNamingNumber, isSplitLesion, isNodal } = measurement;
        ...
        if (measurements.length) {
            ...
            const lesionType = typeof options.isNodal === 'boolean' ? (options.isNodal ? 'nodal ' : 'extranodal ') : '';
            message = options.message || `Each organ should not have more than ${options.limit} ${lesionType}${increment}target${plural}`;
        }
