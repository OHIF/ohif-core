...
class ConformanceCriteria {
     ...
     *
     * @param {MeasurementApi} measurementApi used to retrieve measurements
     * @param {TimepointApi} timepointApi used to retrieve timepoints
     */
    constructor(measurementApi, timepointApi) {
        ...
        this.debouncedValidate = _.debounce((trialCriteriaType, timepointIds) => {
    ...
    async getTimepointNonConformities(trialCriteriaType, timepointId) {
        ...
        const measurementData = await this.getTimepointMeasurementData(timepointId, types);
        const { timepointType } = timepoint;

        // Run validation for this timepoint based on it's type
        let nonconformities = this.validateTimepoint(timepointType, trialCriteriaType, measurementData);
        ...
        if (timepointType === 'followup') {
            ...
            resultBoth = this.validateTimepoint('both', trialCriteriaType, measurementData);
        }
    ...
    async getTimepointMeasurementData(timepointId, types) {
        return new Promise((resolve, reject) => {
            ...
            const current = this.timepointApi.current();
            if (!current) {
                return resolve(data);
            }
            ...
            const fillData = measurementType => {
                const measurements = this.measurementApi.fetch(measurementType, { timepointId });
            ...
            Promise.all(studyPromises.map(p => p.catch(() => null))).then(() => {
                resolve(data);
            }).catch(reject);
        });
    }
    ...
    setMaximums(trialCriteriaType) {
        ...
        overallMaxNewTargets = isFinite(overallMaxNewTargets) ? overallMaxNewTargets : null;

        this.maxTargets.set(overallMaxTargets);
        this.maxNewTargets.set(overallMaxNewTargets);
    }
    ...
    getEvaluators(timepointType, trialCriteriaType) {
        ...
        if (evaluation) {
            ...
            const evaluationTimepoint = timepointType === 'prebaseline' ? evaluation.baseline : evaluation[timepointType];
    ...
    getBaselineMeasurementData(types) {
        const timepoint = this.timepointApi.baseline();
