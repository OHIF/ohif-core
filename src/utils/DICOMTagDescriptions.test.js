...
describe('DICOMTagDescriptions', () => {
  const initialTagDescriptionMap = {
  ...
  test('should return true on DICOMTagDescriptions.isValidTagNumber if it is a valid number or it is <= 0xffffffff', () => {
    expect(DICOMTagDescriptions.isValidTagNumber(1)).toBe(true);
  });

  test('should return false on DICOMTagDescriptions.isValidTagNumber if it is not a valid number', () => {
    expect(DICOMTagDescriptions.isValidTagNumber('house')).toBe(false);
  });

  test('should return false on DICOMTagDescriptions.isValidTagNumber if it is not a valid number hex number', () => {
    expect(DICOMTagDescriptions.isValidTagNumber('1AG')).toBe(false);
  });

  test('should return false on DICOMTagDescriptions.isValidTagNumber if it is a valid number hex number', () => {
    expect(DICOMTagDescriptions.isValidTagNumber(0xaaa)).toBe(true);
  });

  test('should return false on DICOMTagDescriptions.isValidTagNumber if it the number is undefined', () => {
    expect(DICOMTagDescriptions.isValidTagNumber(undefined)).toBe(false);
  });

  test('should return false on DICOMTagDescriptions.isValidTagNumber if it the number is null', () => {
    expect(DICOMTagDescriptions.isValidTagNumber(null)).toBe(false);
  });

  test('should return false on DICOMTagDescriptions.isValidTagNumber if it the number is not defined', () => {
    expect(DICOMTagDescriptions.isValidTagNumber()).toBe(false);
  });

  test('should return true on DICOMTagDescriptions.isValidTag if the tag is a valid', () => {
    expect(DICOMTagDescriptions.isValidTag('x00020000')).toBe(true);
  });

  test('should return false on DICOMTagDescriptions.isValidTag if it the tag is not valid', () => {
    expect(DICOMTagDescriptions.isValidTag('NOT_VALID_TAG')).toBe(false);
  });

  test('should return true on DICOMTagDescriptions.isValidTag for all tags defined within this test', () => {
    Object.keys(initialTagDescriptionMap).forEach(tag => {
      expect(DICOMTagDescriptions.isValidTag(tag)).toBe(true);
    });
  });

  test('should return a tag object on DICOMTagDescriptions.find if the parameter is a valid tag description', () => {
    const tagFound = { keyword: 'FileMetaInfoGroupLength', tag: 'x00020000' };
    expect(DICOMTagDescriptions.find('FileMetaInfoGroupLength')).toEqual(
      tagFound
    );
  });

  test('should be able to initialize the DICOMTagDescriptions successfuly', () => {
    jest.mock('./DICOMTagDescriptions');

    const DICOMTagDescriptions = { init: jest.fn() };
    DICOMTagDescriptions.init();

    expect(DICOMTagDescriptions.init).toHaveBeenCalled();
  });
});
